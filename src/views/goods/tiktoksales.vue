<template>
  <div class="box">
    ...
    <el-table v-loading="loading" :data="tableData" class="sticky-head" @sort-change="handlesortChange" @row-click="jumpurlShow" ref="tableMain">
          ...
          <div class="info rdx-flex info-two">
            ...
            <div class="rdx-flex-col rdx-m-l-10">
              <div class="rdx-flex rdx-flex-wrap">
                <span class="name rdx-m-r-4">{{ row.title }}</span>
              </div>
            </div>
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="coupon_price" align="center" label="价格(元)" width="120" sortable="custom">
        <template slot-scope="scope">
          <div class="flex w-100 justify-center">
            <p class="pricenow">{{ tableData[scope.$index].coupon_price }}</p>
            <p class="pricebefore rdx-tips-color rdx-p-l-10">{{ tableData[scope.$index].market_price }}</p>
          </div>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="rate" sortable="custom" label="佣金比例" width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.rate }}%</span>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="day_order_count" sortable="custom" label="昨日销量(件)"  width="130">
      </el-table-column>
      <el-table-column align="center" prop="amount" sortable="custom" label="销售额(元)" width="130"></el-table-column>
      <el-table-column align="center" prop="order_count" sortable="custom" label="月销量(件)"  width="120"></el-table-column>
      <el-table-column align="center" prop="month_conversion_rate" sortable="custom" label="转化率"  width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.month_conversion_rate }}%</span>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="amount2" label="操作" width="80">
        <template slot-scope="scope">
          <div class="flex w-100 justify-center">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async loadtikSaletData() {
        this.params.page = 1
        this.loading = true
        const {
          data
        } = await this.$get(tikSaleRank, this.params)
        this.tableData = []
        if(data!=undefined){
          this.loading = false
          this.params.page = 2
