<template>
  <div class="box">
    ...
    <el-table v-loading="loading" :data="tableData" class="sticky-head"
      @sort-change="handlesortChange" @row-click="jumpurlShow" ref="tableMain">
      <template slot="empty">
      ...
      <el-table-column label="商品" min-width="220">
        <template slot-scope="{ row }">
          <div class="info rdx-flex info-three">
            ...
            <div class=" rdx-m-l-10">
              <div class="rdx-flex rdx-flex-wrap">
                <span class="name rdx-m-r-4 font-strong rdx-main-color">{{ row.title }}</span>
              </div>
            </div>
          </div>
        </template>
      ...
      <el-table-column align="center" label="价格(元)" width="120">
        <template slot-scope="scope">
          <div class="flex justify-center">
            ...
            <p class="pricebefore rdx-p-l-10 rdx-tips-color">{{ tableData[scope.$index].market_price }}</p>
          </div>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="rate" sortable="custom" label="佣金比例" width="120">
        <template slot-scope="scope">
          <span>{{ tableData[scope.$index].rate }}%</span>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="author_number" sortable="custom" label="昨日带货达人" width="140">
      </el-table-column>
      ...
      <el-table-column align="center" prop="month_conversion_rate" sortable="custom" label="转化率" width="120">
        <template slot-scope="scope">
          <span>{{ tableData[scope.$index].month_conversion_rate }}%</span>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async loadtikhotData() {
        this.params.page = 1
        this.loading = true

        const {
          data
        } = await this.$get(tikHotRank, this.params)
        this.tableData = []
        if (data != undefined) {
          this.loading = false
          this.params.page = 2
          ...
          this.tableData.forEach((item)=>{
            item.month_conversion_rate = item.month_conversion_rate.toFixed(2)
          })
