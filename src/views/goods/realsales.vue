<template>
  <div class="box">
    ...
    <el-table v-loading="loading" :data="tableData" class="sticky-head" @sort-change="handlesortChange" @row-click="jumpurlShow">
      ...
      <el-table-column prop="sale_incr" label="近两小时销量(件)" width="160" align="center" sortable="custom"></el-table-column>
...
<script>
  ...
  export default {
    components: { dateDay,goodsClass,noData, },
    data() {
      return {
        search: '',
        isChoose: true,
    ...
    methods: {
      ...
      async loadRealSalesData() {
        ...
        const {
          data
        } = await this.$get('/api/rank/home/realSalesRank', this.params)
        this.tableData = []
        if(data!=undefined){
          this.params.page = 2
          this.loading = false
