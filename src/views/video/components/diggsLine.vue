<template>
  <div class="w-100">
    ...
    <div v-if="!show.dataShow" id="diggsIds" style="width: 100%; height: 350px;"></div>
    <slot></slot>
  </div>
</template>
      ...
      async getLike() {
        ...
        if (data != undefined) {
          ...
          this.thumbAddCur.forEach((item) => {
            addDiggCount.push(item.digg_count)
          })
          this.thumbTotalCur.forEach((item) => {
            ...
            totalDiggCount.push(item.digg_count)
          })

          //console.log('sdadxc',dates,addDiggCount)
          var chartDom = document.getElementById('diggsIds')
