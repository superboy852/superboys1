<template>
  <div class="w-100">
    ...
    <div v-if="!show.dataShow" id="commonIds" style="width: 100%; height: 350px;"></div>
    <slot></slot>
  </div>
</template>
      ...
      async getComment() {
        ...
        if (data != undefined) {
          ...
          this.thumbAddCur.forEach((item) => {
            addCommentCount.push(item.comment_count)
          })
          this.thumbTotalCur.forEach((item) => {
            ...
            totalCommentCount.push(item.comment_count)
          })

          var chartDom = document.getElementById('commonIds')
