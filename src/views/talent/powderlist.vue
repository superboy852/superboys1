...
<!--            <span class="rdx-m-l-16 rdx-m-t-4">
          ...
          <div class="info rdx-flex info-three">
            ...
            <div class="rdx-flex-col rdx-m-l-10" style="font-weight: normal;">
              <div class="rdx-flex rdx-flex-wrap">
                <span class="name rdx-m-r-4 rdx-line-1" style="width: 100px;">{{ row.nickname }}</span>
                <i class="iconfont icon-e951 rdx-font-14" style="margin-left: 10px;margin-right: 8px;"></i>
                <i class="iconfont icon-e610 rdx-primary rdx-font-14 flex-direction"></i>
              </div>
              <div class="dyids rdx-p-t-6">
                <span>抖音号:</span>
      ...
      <el-table-column align="center" label="认证" width="160">
        <template slot-scope="{ row }">
          <div class="flex align-center justify-center">
            <i class="iconfont rdx-font-20" style="margin-top: 3px;" :class="row.verifycation_type== '0'? '': row.verifycation_type == '1' ? 'icon-e718 rdx-warning-dark' :'icon-e718 rdx-primary-dark'"></i>
            <span class="name rdx-m-l-10">{{row.verify_name}}</span>
          </div>
          <!-- <span>{{row.labels}}</span> -->
        </template>
      </el-table-column>
      <el-table-column align="center" prop="follower_incr" label="粉丝增量" sortable="custom"width="130"></el-table-column>
      <el-table-column align="center" prop="follower_count" label="粉丝数" sortable="custom" width="130"></el-table-column>
      <el-table-column align="center" prop="product_count" label="商品数" sortable="custom"width="130"></el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      isCategoryAll: false,
      isTagAll: false,
      params: { page: 1, size: 10,starCategory:'',day:'',dayType:''},
      categoriesArr: [],
      checkList: [],
      category:[],
      tableData: [],
  ...
  methods: {
    ...
    async loadData() {
      this.params.page = 1
      this.loading = true
      const { data } = await this.$get('/api/rank/author/fans',this.params)
      this.tableData = []
      if(data!=undefined){
        this.loading=false
        this.params.page = 2
    ...
    async getMore() {
      const {
        data
      } = await this.$get('/api/rank/author/fans', this.params)
      if (this.tokenOk == '51000') {
        this.show.tableFoot = '1' //需要登录
        return;
      } else if (this.tokenOk == '52000') {
