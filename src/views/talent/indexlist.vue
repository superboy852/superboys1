<template>
  <div class="box">
    ...
    <el-table v-loading="loading" :data="tableData" class="sticky-head" @row-click="jumpurl" @sort-change="handlesortChange">
      ...
      <el-table-column align="center" label="排行" type="index" width="160">
...
<!--            <span class="rdx-m-l-16 rdx-m-t-4">
      ...
      <el-table-column align="center" prop="growth_compon" label="成长指数" sortable="custom" width="120"></el-table-column>
      <el-table-column align="center" prop="follower_count_incr" label="粉丝增量" sortable="custom" width="120"></el-table-column>
      <el-table-column align="center" prop="follower_count" label="粉丝数" sortable="custom" width="120"></el-table-column>
      <el-table-column align="center" prop="digg_count_incr" label="点赞增量" sortable="custom" width="120"></el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      this.params.page = 1
      this.loading = true
      const { data } = await this.$get('/api/rank/author/componAuthorRanking',this.params)
      this.tableData = []
      if(data!=undefined){
        this.loading=false
        this.params.page = 2
    ...
    async getMore() {
      const {
        data
      } = await this.$get('/api/rank/author/componAuthorRanking', this.params)
      if (this.tokenOk == '51000') {
        this.show.tableFoot = '1' //需要登录
        return;
      } else if (this.tokenOk == '52000') {
