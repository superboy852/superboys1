<template>
  <div class="box">
    ...
    <el-table v-loading="loading" :data="tableData" class="sticky-head" @row-click="jumpurl">
      <template slot="empty">
...
<!--            <span class="rdx-m-l-16 rdx-m-t-4">
      ...
      <el-table-column label="达人">
        <template slot-scope="{ row }">
          <div class="info rdx-flex info-three" style="font-weight: normal;">
            <img :src="row.avatar" alt class="circularHead" />
            <div class="rdx-flex-col rdx-m-l-16">
      ...
      <el-table-column align="center" prop="forward_count" label="转发数" width="120"></el-table-column>
      <el-table-column align="center" prop="follower_count" label="粉丝数" width="120"></el-table-column>
      <el-table-column align="center" prop="digg_count_incr" label="点赞增量" width="120"></el-table-column>
      <el-table-column align="center" prop="comment_count_incr" label="评论增量" width="120"></el-table-column>
      <el-table-column align="center" prop="share_incr" label="转发增量" width="120"></el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      isCategoryAll: false,
      isTagAll: false,
      params: { page: 1, size: 10,starCategory:""},
      categoriesArr: [],
      checkList: [],
      category:[],
      tableData: [],
  ...
  methods: {
    ...
    async loadData() {
      this.params.page = 1
      this.loading = true
      const { data } = await this.$get('/api/rank/author/shareAuthorRanking',this.params)
      this.tableData = []
      if(data!=undefined){
        this.loading=false
        this.params.page = 2
    ...
    async getMore() {
      const {
        data
      } = await this.$get('/api/rank/author/shareAuthorRanking', this.params)
      if (this.tokenOk == '51000') {
        this.show.tableFoot = '1' //需要登录
        return;
      } else if (this.tokenOk == '52000') {
