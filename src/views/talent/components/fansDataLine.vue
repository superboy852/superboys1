<template>
  <div class="w-100">
    ...
    <div v-if="!show.dataShow" id="fanDataIds" style="width: 100%; height: 350px;"></div>
    <slot></slot>
  </div>
</template>
      ...
      async getFans() {
        ...
        if (data != undefined) {
          ...
          this.fansClubAdd.forEach((item) => {
            fansAdd.push(item.fans_club_total)
          })
          this.fansClubTotal.forEach((item) => {
            item.date = tools.time_cycle(5, item.date)
            item.date = item.date.replace('-', '/')
            dates.push(item.date)
            fansTotal.push(item.fans_club_total)
          })


          var chartDom = document.getElementById('fanDataIds')
